...
const { REDIS_CONFIG } = require('../util/constants.js');

const { MQTT_DEVICE_USER, MQTT_DEVICE_PASSWORD, BROKER_PORT } = process.env;

const port = BROKER_PORT || 1883;

const getClientRole = ({ username, password }) => {
  if (username === MQTT_DEVICE_USER && password === MQTT_DEVICE_PASSWORD) {
    return 'admin';
  }
